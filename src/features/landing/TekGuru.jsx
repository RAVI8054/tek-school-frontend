    ...
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        {
          id: Date.now() + 1,
          role: 'assistant',
          text: `I can help with that! Since this is a demo, I'm just a mock response. But in the real app, I'll guide you through "${v}".`,
        }
      ]);
      setIsLoading(false);
    }, 1500);
            ...
            <div className="flex flex-wrap gap-2">
              {STARTERS.map((s) => (
                <button
                  key={s}
                  onClick={() => send(s)}
                  className="rounded-full bg-accent-blue/15 px-3.5 py-1.5 text-xs font-semibold text-[var(--accent-blue-deep)] transition-transform hover:scale-[1.03]"
                >
                  {s}
                </button>
              ))}
