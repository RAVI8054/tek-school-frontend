...
export function CommunityDetail({ channel, onClose }) {
  ...
  const handleBlockUser = async (userId, userName) => {
    if (!window.confirm(`Are you sure you want to block ${userName} from community access?`)) return;
    
    try {
      const res = await blockAdminCommunityUser(userId);
      if (res.status === 'success') {
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <div className="flex opacity-0 group-hover:opacity-100 transition-opacity gap-1">
                      {msg.senderId && (
                        <button
                          onClick={() => handleBlockUser(msg.senderId._id, msg.senderId.name)}
