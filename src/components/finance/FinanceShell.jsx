...
const NAV_GROUPS = [
  ...
  { label: 'System', items: [
    ...
    { to: '/finance/audit-logs', label: 'Audit Logs', icon: Shield },
  ] }
];
            ...
            <div className="flex items-center gap-2 px-3 py-2 sm:gap-3 sm:px-4 md:px-6">
              ...
              {!hideDefaultSearch && (
                <div className="relative hidden max-w-xs flex-1 md:block">
                  ...
                  <input placeholder="Search payments, students…" className="h-9 w-full rounded-lg border border-slate-200 bg-white pl-9 pr-3 text-xs outline-none focus:border-[var(--accent-blue-deep)]" />
                </div>
              )}
              {actions}
              <button
                onClick={() => setAiOpen(true)}
                className="hidden items-center gap-1.5 rounded-lg bg-gradient-to-r from-[#065F46] to-[#047857] px-3 py-1.5 text-xs font-semibold text-white shadow-[0_6px_16px_-8px_#065F46] md:inline-flex"
              >
